...
function createRoom(hostId) {
  ...
  const room = {
    ...
    spectators: [],          // mehmonlar (kuzatuvchilar)
...
function activePlayers(room) { return room.players.filter(p => !p.eliminated && !p.left); }
...
function stateFor(room, me) {
  const st = room.st;
  const spec = !!me.isSpectator;
  const base = {
    spectator: spec,
    spectators: (room.spectators || []).filter(x => x.connected).map(x => x.name),
    t: 'state',
    code: room.code,
    phase: room.phase,
    roundNumber: room.roundNumber,
    youId: me.id,
    isHost: !spec && me.id === room.hostId,
    ...
    canRestart: !spec && !me.usedRestart && !room.pendingVote,
    canEnd: !spec && !me.usedEnd && !room.pendingVote,
    pendingVote: room.pendingVote ? {
      ...
      youNeedToVote: !spec && room.pendingVote.proposerId !== me.id && !room.pendingVote.votes[me.id] && me.connected,
  ...
  if (room.phase === 'playing' && st) {
    ...
    base.yourTurn = !spec && currentId(room) === me.id && !me.foldedThisRound;
    base.you = { hand: spec ? [] : (me.hand || []) };
    base.youFolded = !spec && me.foldedThisRound;
...
function broadcastState(room) {
  for (const p of room.players) send(p, stateFor(room, p));
  for (const sp of (room.spectators || [])) send(sp, stateFor(room, sp));
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('close', () => {
    ...
    const sp = findSpec(room, playerId);
    if (sp) { sp.connected = false; sp.ws = null; }
...
// O'lik ulanishlarni aniqlash (har 15s) — telefon fonga o'tsa soket "zombi" bo'lib qoladi
setInterval(() => {
...
function handleMessage(ws, m) {
  switch (m.t) {
    case 'ping': { try { ws.send(JSON.stringify({ t: 'pong' })); } catch (_) {} return; }
    ...
    case 'watch': return onWatch(ws, m);
    case 'quit': return withRoom(ws, (room, p) => handleQuit(room, p));
...
function withRoom(ws, fn) {
  ...
  if (!room) return;
  const p = findPlayer(room, ws.meta.playerId) || findSpec(room, ws.meta.playerId);
  if (!p) return;
  fn(room, p);
}
...
/* --------- MEHMON (kuzatuvchi) --------- */
function findSpec(room, id) { return (room.spectators || []).find(x => x.id === id); }

function onWatch(ws, m) {
  const code = String(m.code || '');
  const room = rooms.get(code);
  if (!room) { try { ws.send(JSON.stringify({ t: 'error', msg: 'Bunday xona topilmadi.' })); } catch (_) {} return; }
  const id = String(m.tgId || rndId());
  const name = (m.name || 'Mehmon').slice(0, 16);

  // Agar o'yinchi sifatida ro'yxatda bo'lsa — oddiy qayta ulanish
  const asPlayer = findPlayer(room, id);
  if (asPlayer) { asPlayer.ws = ws; asPlayer.connected = true; ws.meta = { roomCode: code, playerId: id }; broadcastState(room); return; }

  let sp = findSpec(room, id);
  if (sp) { sp.ws = ws; sp.connected = true; sp.name = name; }
  else { sp = { id, name, ws, connected: true, isSpectator: true }; room.spectators.push(sp); }
  ws.meta = { roomCode: code, playerId: id, spectator: true };
  addLog(room, `${name} kuzatuvchi sifatida qo'shildi.`);
  sendChatHistory(sp, room);
  broadcastState(room);
}

/* --------- IXTIYORIY CHIQISH: o'yin faqat SHU o'yinchi uchun yakunlanadi --------- */
function handleQuit(room, player) {
  if (player.left) return;
  // Qo'lidagi kartalar ochkoga qo'shiladi (halol yakun)
  if (room.phase === 'playing' && player.hand && player.hand.length) {
    player.cumulative += handScore(player.hand);
  }
  player.left = true;
  player.eliminated = true;
  player.foldedThisRound = true;
  player.hand = [];
  leaveCall(room, player);
  addLog(room, `${player.name} o'yindan chiqdi (yakuniy ochko: ${player.cumulative}).`);

  // Xona egasi chiqsa — keyingi o'yinchiga o'tkazamiz
  if (room.hostId === player.id) {
    const next = room.players.find(p => !p.left && !p.eliminated && p.connected && !p.isBot)
              || room.players.find(p => !p.left && !p.eliminated);
    if (next) { room.hostId = next.id; addLog(room, `Xona egaligi ${next.name} ga o'tdi.`); }
  }

  // Chiqqan odamga yakuniy natija
  send(player, { t: 'quit-ok', cumulative: player.cumulative, place: quitPlace(room, player) });

  // Navbat unda bo'lsa — suramiz
  if (room.phase === 'playing' && room.st && currentId(room) === player.id) advance(room, 1);

  // Raundda bitta o'yinchi qolsa — raund yakuni; umuman 1 kishi qolsa — o'yin tugaydi
  const active = activePlayers(room);
  if (active.length <= 1) { finishGame(room); return; }
  if (room.phase === 'playing') {
    const inRound = inRoundPlayers(room);
    if (inRound.length === 1) { endRound(room, inRound[0].id, null); return; }
  }
  broadcastState(room);
}

function quitPlace(room, player) {
  const all = room.players.slice().sort((a, b) => a.cumulative - b.cumulative);
  return all.findIndex(p => p.id === player.id) + 1;
}
...
function newPlayer(id, name, ws) {
  return {
    ...
    foldedThisRound: false, usedRestart: false, usedEnd: false, roundDelta: 0, left: false,
  };
}
